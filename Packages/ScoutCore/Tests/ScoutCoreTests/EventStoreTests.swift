...
struct EventStoreTests {
    ...
    func appendAndRead() async throws {
        ...
        let receipts = try await store.append(events)
    ...
    func atomicRollback() async throws {
        let store = InMemoryEventStore()
        let validFirst = try ScoutFixtures.sampleEvents()[0]
        let invalidSecond = ScoutEventEnvelope.seal(
            ...
            payload: validFirst.payload,
            previousHash: nil
        )

        do {
            _ = try await store.append([validFirst, invalidSecond])
    ...
    func duplicateEventID() async throws {
        ...
        do {
            ...
            #expect(error == .duplicateEventID(first.id))
        }
