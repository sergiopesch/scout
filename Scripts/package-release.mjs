...
const buildNumber = process.env.SCOUT_BUILD_NUMBER ?? "1";
...
const zip = join(workspace, "dist", `Scout-${version}-macOS.zip`);
const dmg = join(workspace, "dist", `Scout-${version}-macOS.dmg`);
const notarization = {};
...
async function notarize(path, label) {
  ...
  const logPath = join(workspace, "dist", `notarization-${label}.json`);
  await run("xcrun", [
...
}).then(({ stdout }) => writeFile(join(workspace, "Gateway", "sbom.cdx.json"), stdout));
...
await copyFile(join(workspace, "Gateway/sbom.cdx.json"), join(resources, "gateway-sbom.cdx.json"));
...
const dmgStaging = join(workspace, "dist", `.dmg-${process.pid}`);
